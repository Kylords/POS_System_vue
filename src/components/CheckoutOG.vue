...
const { result: infoResult, onResult: onInfoResult, refetch: refetchInfo } = useQuery(
    gql`
    query($email: String!) {
  findLatestOrderEmails(email: $email){
    email
    firstName
    lastName
    mobile
        ...
        <div>
            <div class="col-md-3">
                ...
                <div class="input-div row">
                    ...
                    <input type="text" v-model="placeOrder.email" placeholder="Email" class="form-control" @change="">
                    <ul v-if="emailSuggestions.length > 0 && placeOrder.email !== '' && infoResult"
                        ...
                        <li class="suggestion-product" v-for="email in emailSuggestions.filter(item => item.email.toLowerCase().includes(placeOrder.email))" :key="email.id"
                            @click="clickEmail(email)" @change="showSuggestedEmails = true">
                            {{ email.email }}
